...
use gstpup::inference::{YoloPostProcessor, ModelPostProcessor, InferenceError};
...
fn test_yolo_output_format_assumptions() {
    ...
    if !detections.is_empty() {
        ...
        println!("Output detection: x1={}, y1={}, x2={}, y2={}, score={}, class={}", 
        ...
        if output_center_x != 320.0 || output_center_y != 320.0 {
            ...
            println!("Expected center: (320, 320), Got: ({}, {})", output_center_x, output_center_y);
        }
    }
...
fn test_multiple_detections_coordinate_explosion() {
    ...
    for (i, det) in detections.iter().enumerate() {
        if det.x1 < 0.0 || det.y1 < 0.0 || det.x2 > 640.0 || det.y2 > 640.0 {
            ...
            println!("Detection {}: OUT OF BOUNDS - ({}, {}, {}, {})", 
    ...
    println!("Valid detections: {}, Out of bounds: {}", valid_count, out_of_bounds_count);
    
    if out_of_bounds_count > 0 {
        println!("🚨 FOUND {} OUT-OF-BOUNDS DETECTIONS - THIS CONFIRMS THE BUG!", out_of_bounds_count);
    }
}
...
fn test_confidence_threshold_behavior() {
    ...
    if !detections.is_empty() {
        println!("🚨 LOW CONFIDENCE DETECTION PASSED THROUGH! Score: {}", detections[0].score);
