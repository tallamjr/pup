...
use gstpup::{
    AppConfig, ConsoleReporter, InferenceConfig, InputConfig, Metrics, ModeConfig, OutputConfig,
    PerformanceMonitor, PreprocessingConfig, PupError,
};
...
mod coreml_optimization_tests {
    ...
    fn test_coreml_roadmap_success_criteria() {
        ...
        assert!(
            metrics.get_inference_latency_ms() < 50.0,
        ...
        assert!(
            metrics.get_memory_usage_mb() < 500,
        ...
        assert!(
            metrics.check_performance_targets(30.0, 50.0).is_ok(),
